...
export class uploadHelper {
    ...
    static addPasteEventListener(elementId: string, fileId: string) {
        if (elementId === null) {
            document.addEventListener('paste', (e: ClipboardEvent) => uploadHelper.pasteHandler(e, fileId));
        } else {
            const element = document.getElementById(elementId) as HTMLElement;
            element.addEventListener('paste', (e: ClipboardEvent) => uploadHelper.pasteHandler(e, fileId));
        }
    }
    static removePasteEventListener(elementId: string, fileId: string) {
        if (elementId === null) {
            document.removeEventListener('paste', (e: ClipboardEvent) => uploadHelper.pasteHandler(e, fileId));
        } else {
            const element = document.getElementById(elementId) as HTMLElement;
            element.removeEventListener('paste', (e: ClipboardEvent) => uploadHelper.pasteHandler(e, fileId));
        }
    }
    private static pasteHandler(e: ClipboardEvent, fileId: string) {
        const fileElement = document.getElementById(fileId) as HTMLInputElement;
        if (e.clipboardData === undefined || e.clipboardData.files.length === 0) {
            return;
        };
        fileElement.files = e.clipboardData.files;
