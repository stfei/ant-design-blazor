...
export class uploadHelper {
    ...
    static addPasteEventListener(element: HTMLElement, input: HTMLInputElement, instance: any) {  
        const inputTarget = element.querySelector('input:not([type="file"])');
        ...
        if (!inputTarget && !textareaTarget) {
            document.addEventListener("paste", (e: ClipboardEvent) => uploadHelper.handlePaste(e, input, instance));
        }
    ...
    static removePasteEventListener(element: HTMLElement) {        
        ...
        if (!inputTarget && !textareaTarget) {
            document.removeEventListener("paste", uploadHelper.handlePaste as any);
        }
    }
