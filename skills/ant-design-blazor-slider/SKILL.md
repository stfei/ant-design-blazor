---
name: ant-design-blazor-slider
description: 使用自包含的多种 Razor 用法实现、修改和审查 Ant Design Blazor Slider 组件。覆盖基础配置、状态变化、数据绑定、事件、模板与该组件的代表性高级模式；用于需要比较不同写法并选择正确模式的任务，无需访问源码仓库或文档站。
---

# Ant Design Blazor Slider
